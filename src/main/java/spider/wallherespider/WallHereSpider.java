...
public class WallHereSpider extends BaseSpider<ResourcePageBean, WallHereArgsBean> {
	...
	public ResourcePageBean parseData(Document document) {
		 ...
		 * 原图:https://get.wallhere.com/photo/+缩略图图片名字
		 */
		...
		try {
			...
			resourcePageBean.setTotalPage((int) (total / 24F));
		} catch (Exception e) {
			e.printStackTrace();
		}
		// 获取图片信息
		...
		for (Element element : targetElements) {
			...
			String thumbUrl = tempThumpUrl.substring(0, tempThumpUrl.length() - 2);
			// 中图
			String middleImageUrl = thumbUrl + "!f";
			imageBean.setThumbUrl(middleImageUrl);
			// 原图
			String originBaseUrl = "https://get.wallhere.com/photo/";
			String imgName = thumbUrl.substring(thumbUrl.lastIndexOf("/") + 1, thumbUrl.length());
			String originUrl = originBaseUrl + imgName;
			imageBean.setObjUrl(originUrl);
			// 图片标题
